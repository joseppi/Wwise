...
bool installtex(int tnum, char *texname, int &xs, int &ys, bool clamp)
{
	void * img = NULL;
	int imgx = 0, imgy = 0;
	if ( !readjpeg( texname, imgx, imgy, img ) )
		return false;
	...
	for( xs = imgx, ys = imgy; xs > glmaxtexsize; xs /= 2, ys /= 2);
	for( ; ys> glmaxtexsize; xs /=2, ys /= 2 );

	void *scaledimg = img;
	if(xs!=imgx)
	{
		conoutf("warning: quality loss: scaling %s", texname);     // for voodoo cards under linux
		scaledimg = malloc(xs*ys*3);
		gluScaleImage(GL_RGB, imgx, imgy, GL_UNSIGNED_BYTE, img, xs, ys, GL_UNSIGNED_BYTE, scaledimg);
	};
	if(gluBuild2DMipmaps(GL_TEXTURE_2D, 3, xs, ys, GL_RGB, GL_UNSIGNED_BYTE, scaledimg)) fatal("could not build mipmaps");
	if(img!=scaledimg) free(scaledimg);
	free( img );
	return true;
};
...
void purgetextures()
{
	...
	loopi(256)
	{
		loop(j,MAXFRAMES) mapping[i][j] = 0;
		mapmaterial[i] = 0;
	}
}
...
void texturematerial(char *aframe, char *name, char *material)
{
	int num = curtexnum++, frame = atoi(aframe);
	if(num<0 || num>=256 || frame<0 || frame>=MAXFRAMES) return;
	mapping[num][frame] = 1;
	...
	mapmaterial[num] = snd_getmaterialid( material );
};

COMMAND(texturereset, ARG_NONE);
COMMAND(texture, ARG_2STR);
COMMAND(texturematerial, ARG_3STR);

int lookupmaterial( int tex )
{
	if ( mapmaterial[ tex ] == 0 )
	{
		char szMessage[ 256 ];
		sprintf( szMessage, "Missing material for %s", mapname[ tex ][0] );
		snd_message( szMessage );
	}
	return mapmaterial[ tex ];
}
...
#ifdef __APPLE__
void overbright(float amount) { if(hasoverbright) glTexEnvf(GL_TEXTURE_ENV, GL_RGB_SCALE, amount ); };
#else
