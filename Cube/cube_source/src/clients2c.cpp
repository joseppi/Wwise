...
void updatepos(dynent *d)
{
    ...
    int lagtime = (int) ( lastmillis-d->lastupdate );
    if(lagtime)
    {
...
void localservertoclient(uchar *buf, int len)   // processes any updates from the server
{
    ...
    while(p<end) switch(type = getint(p))
    {
        ...
        case SV_DAMAGE:             
        {
			...
			{
				snd_event(AK::EVENTS::PAIN, getclient(target));
			}
            break;
        };
        ...
        case SV_DIED:
        {
            ...
            snd_event(AK::EVENTS::DEATH_OGRE, d);
            d->lifesequence++;
            break;
        };
        ...
        case SV_ITEMSPAWN:
        {
            ...
            snd_event(AK::EVENTS::ITEMSPAWN, &v); 
            break;
        };
        ...
        case SV_PONG: 
            addmsg(0, 2, SV_CLIENTPING, player1->ping = (player1->ping*5+(int)lastmillis-getint(p))/6);
            break;
