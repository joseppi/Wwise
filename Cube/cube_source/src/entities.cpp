...
void renderent(entity &e, char *mdlname, float z, float yaw, int frame = 0, int numf = 1, float basetime = 0, float speed = 10.0f)
{
    ...
    loopv(ents)
    {
        ...
        else
        {
            ...
            if(e.type!=CARROT)
            {
				...
				renderent(e, entmdlnames[e.type-I_SHELLS], (float)(1+sin(lastmillis/100.0f+e.x+e.y)/20), lastmillis/10.0f);
            }
			else switch(e.attr2)
            {			
				case 1:
                ...
                case 0:
					...
					renderent(e, "carrot", (float)(1+sin(lastmillis/100.0f+e.x+e.y)/20), lastmillis/(e.attr2 ? 1.0f : 10.0f));
					break;
...
struct itemstat { int add, max; int event; } itemstats[] =
{
	 10,    50, AK::EVENTS::ITEMAMMO,
     20,   100, AK::EVENTS::ITEMAMMO,
      5,    25, AK::EVENTS::ITEMAMMO,
      5,    25, AK::EVENTS::ITEMAMMO,
     25,   100, AK::EVENTS::ITEMHEALTH100,
     50,   200, AK::EVENTS::ITEMHEALTH200,
    100,   100, AK::EVENTS::ITEMARMOUR100,
    150,   150, AK::EVENTS::ITEMARMOUR150,
  20000, 30000, AK::EVENTS::ITEMPUP,
};
...
void checkquad(float time)
{
    if(player1->quadmillis && (player1->quadmillis -= (int)time)<0)
    {
        player1->quadmillis = 0;
        snd_clientevent(AK::EVENTS::PUPOUT);
