...
void loadgamerest()
{
    ...
    loopv(monsters)
    {
        ...
        monsters[i]->lastaction = monsters[i]->trigger = lastmillis+500.0f;    // also lazy, but no real noticable effect on game
...
void incomingdemodata(uchar *buf, int len, bool extras)
{
    ...
    gzputi((int) ( lastmillis-starttime ));
    gzputi(len);
    gzwrite(f, buf, len);
    gzput(extras);
    if(extras)
    {
        ...
        gzputi((int) ( player1->lastaction-starttime ));
...
float scaletime(float t) { return (t*(100.0f/demoplaybackspeed))+starttime; };

void readdemotime()
{   
    if(gzeof(f) || (playbacktime = (float) gzgeti())==-1)
    {
        stopreset();
        return;
    };
...
void demoplaybackstep()
{
    while(demoplayback && lastmillis>=playbacktime)
    {
        ...
        if(extras = gzget())     // read additional client side state not present in normal network stream
        {
            ...
            target->lastaction = scaletime((float)gzgeti());
    ...
    if(demoplayback)
    {
        float itime = lastmillis-(float)demodelaymsec;
