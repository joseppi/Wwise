...
dynent *basicmonster(int type, int yaw, int state, int trigger, int move)
{
	...
	char entname[256];
	sprintf( entname, "Monster #%i: %s", (int) ++cMonster, t->name );
...
{
	char name[256];
	sprintf( name, "%s_%s", event, monstername );
	snd_event( name, m );
}
	...
	switch (m->GetMonsterState())
    {
        ...
        case M_SLEEP:                       // state classic sp monster start in, wait for visual contact
        {
            ...
            || angle<10)
            {
				...
				{
					char entname[256];
					sprintf( entname, "Monster #%i: %s", (int) ++cMonster, m->name );
...
void monsterpain(dynent *m, int damage, dynent *d)
{
    ...
    if((m->health -= damage)<=0)
    {
		...
		snd_monsterevent( monstertypes[m->mtype].name, "death", m );
